...
CREATE OR REPLACE VIEW informea_documents AS
  SELECT
    ...
    node.nid
  FROM `edw_ascobans_drupal`.node node
    LEFT JOIN `edw_ascobans_drupal`.field_data_field_publication_published_date pdate ON node.nid = pdate.entity_id
    INNER JOIN `edw_ascobans_drupal`.field_data_field_instrument instr ON node.nid = instr.entity_id
    INNER JOIN informea_treaty_machine_name treaty ON (treaty.nid = instr.field_instrument_target_id AND instr.entity_type = 'node')
    LEFT JOIN `edw_ascobans_drupal`.field_data_field_publication_image img ON node.nid = img.entity_id
    LEFT JOIN `edw_ascobans_drupal`.file_managed thumbnails ON field_publication_image_fid = thumbnails.fid
    LEFT JOIN `edw_ascobans_drupal`.field_data_field_country country ON country.entity_id = node.nid
    LEFT JOIN `edw_ascobans_drupal`.field_data_field_country_iso3 ciso ON country.field_country_target_id = ciso.entity_id
  WHERE
    treaty IS NOT NULL
...
CREATE OR REPLACE VIEW informea_documents_types AS
  ...
  SELECT
    ...
    tb.name `value`
  FROM informea_documents a
    INNER JOIN `edw_ascobans_drupal`.field_data_field_publication_type b ON a.nid = b.entity_id
    INNER JOIN `edw_ascobans_drupal`.taxonomy_term_data tb ON b.field_publication_type_tid = tb.tid
...
CREATE OR REPLACE VIEW informea_documents_authors AS
  SELECT
    ...
    tb.name
  FROM informea_documents a
    INNER JOIN `edw_ascobans_drupal`.field_data_field_publication_author b ON a.nid = b.entity_id
    INNER JOIN `edw_ascobans_drupal`.taxonomy_term_data tb ON tb.tid = b.field_publication_author_tid;
...
CREATE OR REPLACE VIEW informea_documents_keywords AS
  ...
  FROM informea_documents a
    INNER JOIN `edw_ascobans_drupal`.field_data_field_cms_tags tags ON tags.entity_id = a.nid
    INNER JOIN `edw_ascobans_drupal`.field_data_field_related_informea_terms itags ON tags.field_cms_tags_tid = itags.entity_id
    INNER JOIN `edw_ascobans_drupal`.taxonomy_term_data td ON itags.field_related_informea_terms_target_id = td.tid;
...
CREATE OR REPLACE VIEW informea_documents_titles AS
  ...
  FROM informea_documents a
    INNER JOIN `edw_ascobans_drupal`.field_data_title_field b ON a.nid = b.entity_id
...
CREATE OR REPLACE VIEW informea_documents_files AS
  SELECT
    ...
    files.filename
  FROM informea_documents a
    INNER JOIN `edw_ascobans_drupal`.field_data_field_publication_attachment f ON a.nid = f.entity_id
    INNER JOIN `edw_ascobans_drupal`.file_managed files ON f.field_publication_attachment_fid = files.fid
  GROUP BY files.fid;
...
CREATE OR REPLACE VIEW `informea_documents_references` AS
  SELECT
    ...
    FROM
      ...
      JOIN `edw_ascobans_drupal`.field_data_field_publication_meeting b ON a.nid = b.entity_id
      JOIN `edw_ascobans_drupal`.node bn ON (b.field_publication_meeting_target_id = bn.nid AND bn.type = 'meeting')
    GROUP BY bn.nid
  UNION
    SELECT
    ...
    FROM
      informea_documents a
      JOIN `edw_ascobans_drupal`.field_data_field_publication_plans b ON a.nid = b.entity_id
      JOIN `edw_ascobans_drupal`.node bn ON (b.field_publication_plans_target_id = bn.nid and (bn.type = 'document'))
    GROUP BY bn.nid
  UNION
    SELECT
    ...
    FROM
      informea_documents a
      JOIN `edw_ascobans_drupal`.field_data_field_publication_nat_report b ON a.nid = b.entity_id
      JOIN `edw_ascobans_drupal`.node bn ON (b.field_publication_nat_report_target_id = bn.nid AND bn.type = 'document')
    GROUP BY bn.nid;
