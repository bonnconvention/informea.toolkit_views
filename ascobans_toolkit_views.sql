...
CREATE OR REPLACE DEFINER=`edw_www`@`localhost` SQL SECURITY DEFINER VIEW `informea_meetings` AS
    ...
    FROM
      `edw_ascobans_drupal`.node a
        ...
        INNER JOIN `edw_ascobans_drupal`.field_data_field_country i ON a.nid = i.entity_id
...
CREATE OR REPLACE DEFINER=`edw_www`@`localhost` SQL SECURITY DEFINER VIEW `informea_decisions` AS
    ...
    FROM `edw_ascobans_drupal`.node a
        ...
        INNER JOIN `edw_ascobans_drupal`.informea_decisions_cop_documents g ON g.id_document = a.nid
    WHERE
        a.`type`='document'
...
CREATE OR REPLACE DEFINER=`edw_www`@`localhost` SQL SECURITY DEFINER VIEW `informea_country_reports_title` AS
    SELECT
        ...
        'en' AS 'language',
        a.title
    FROM `edw_ascobans_drupal`.node a WHERE a.`type` = 'document';
