...
CREATE OR REPLACE VIEW `informea_meetings` AS
  ...
  FROM
    `prod_ascobans`.node a
    INNER JOIN `prod_ascobans`.field_data_field_instrument instr ON a.nid = instr.entity_id
    LEFT JOIN `prod_ascobans`.node instr_name ON instr.field_instrument_target_id = instr_name.nid
    INNER JOIN `prod_ascobans`.field_data_event_calendar_date b ON a.nid = b.entity_id
    LEFT JOIN `prod_ascobans`.field_data_field_meeting_kind d ON a.nid = d.entity_id
    LEFT JOIN `prod_ascobans`.taxonomy_term_data d1 ON d.field_meeting_kind_tid = d1.tid
    INNER JOIN `prod_ascobans`.field_data_field_meeting_type e ON a.nid = e.entity_id
    INNER JOIN `prod_ascobans`.taxonomy_term_data e1 ON e.field_meeting_type_tid = e1.tid
    LEFT JOIN `prod_ascobans`.field_data_field_meeting_status f ON a.nid = f.entity_id
    LEFT JOIN `prod_ascobans`.taxonomy_term_data f1 ON f.field_meeting_status_tid = f1.tid
    LEFT JOIN `prod_ascobans`.field_revision_field_meeting_location g ON a.nid = g.entity_id
    LEFT JOIN `prod_ascobans`.field_data_field_meeting_city h ON a.nid = h.entity_id
    INNER JOIN `prod_ascobans`.field_data_field_country i ON i.entity_id = a.nid
    INNER JOIN `prod_ascobans`.field_data_field_country_iso2 i1 ON i.field_country_target_id = i1.entity_id
    LEFT JOIN `prod_ascobans`.field_data_field_meeting_latitude j ON a.nid = j.entity_id
    LEFT JOIN `prod_ascobans`.field_data_field_meeting_longitude k ON a.nid = k.entity_id
  WHERE
    a.status = 1
    AND a.`type` = 'meeting'
...
CREATE OR REPLACE VIEW `informea_meetings_description` AS
  ...
  FROM `prod_ascobans`.node a
    INNER JOIN `prod_ascobans`.field_data_body b ON a.nid = b.entity_id
  WHERE
    a.status = 1
...
CREATE OR REPLACE VIEW `informea_decisions_cop_documents` AS
  ...
  FROM
    `prod_ascobans`.node a
    INNER JOIN `prod_ascobans`.field_data_field_meeting_type f ON a.nid = f.entity_id
    INNER JOIN `prod_ascobans`.taxonomy_term_data g ON f.field_meeting_type_tid = g.tid
    INNER JOIN `prod_ascobans`.field_data_field_document_meeting h ON h.field_document_meeting_target_id = a.nid
  WHERE
    a.status = 1
    AND a.type = 'meeting'
...
CREATE OR REPLACE VIEW `informea_decisions` AS
  SELECT
    ...
    a.nid
  FROM `prod_ascobans`.node a
    INNER JOIN `prod_ascobans`.field_data_field_document_type b ON b.entity_id = a.nid
    INNER JOIN `prod_ascobans`.taxonomy_term_data b1 ON b.field_document_type_tid = b1.tid
    INNER JOIN `prod_ascobans`.field_data_field_document_status c ON c.entity_id = a.nid
    INNER JOIN `prod_ascobans`.taxonomy_term_data c1 ON c.field_document_status_tid = c1.tid
    INNER JOIN `prod_ascobans`.field_data_field_document_number d ON d.entity_id = a.nid
    INNER JOIN `prod_ascobans`.field_data_field_instrument e ON e.entity_id = a.nid
    INNER JOIN `prod_ascobans`.node e1 ON e.field_instrument_target_id = e1.nid
    INNER JOIN `prod_ascobans`.field_data_field_document_publish_date f ON f.entity_id = a.nid
...
CREATE OR REPLACE VIEW `informea_decisions_documents` AS
  ...
  FROM `prod_ascobans`.node a
    ...
    INNER JOIN `prod_ascobans`.field_data_field_instrument e ON e.entity_id = a.nid
    INNER JOIN `prod_ascobans`.node e1 ON e.field_instrument_target_id = e1.nid
    INNER JOIN `prod_ascobans`.field_data_field_document_files f ON f.entity_id = a.nid
    INNER JOIN `prod_ascobans`.field_data_field_document_file f1 ON f1.entity_id = f.field_document_files_value
    INNER JOIN `prod_ascobans`.file_managed f2 ON f2.fid = f1.field_document_file_fid
  WHERE
    a.status = 1
    AND a.`type` = 'document'
...
CREATE OR REPLACE VIEW `informea_country_reports` AS
  ...
  FROM `prod_ascobans`.node a
    ...
    INNER JOIN `prod_ascobans`.field_data_field_instrument e
      ON (e.entity_id = a.nid AND e.field_instrument_target_id = 4)
    INNER JOIN `prod_ascobans`.field_data_field_document_publish_date f ON f.entity_id = a.nid
    INNER JOIN `prod_ascobans`.field_data_field_country g ON (g.entity_id = a.nid AND g.bundle = 'document')
    INNER JOIN `prod_ascobans`.field_data_field_country_iso3 h ON g.field_country_target_id = h.entity_id
  WHERE
    a.`type` = 'document'
  GROUP BY a.uuid;
...
CREATE OR REPLACE VIEW `informea_country_reports_documents` AS
  SELECT
    ...
    f2.filename AS filename
  FROM `prod_ascobans`.node a
    INNER JOIN `prod_ascobans`.field_data_field_document_type dt ON a.nid = dt.entity_id
    INNER JOIN `prod_ascobans`.field_data_field_document_files f ON f.entity_id = a.nid
    INNER JOIN `prod_ascobans`.field_data_field_document_file f1 ON f1.entity_id = f.field_document_files_value
    INNER JOIN `prod_ascobans`.file_managed f2 ON f2.fid = f1.field_document_file_fid
    INNER JOIN `prod_ascobans`.field_data_field_instrument e ON (e.entity_id = a.nid AND e.field_instrument_target_id = 4)
    INNER JOIN `prod_ascobans`.field_data_field_country g ON (g.entity_id = a.nid AND g.bundle = 'document')
    INNER JOIN `prod_ascobans`.field_data_field_country_iso3 h ON g.field_country_target_id = h.entity_id
  WHERE
    a.status = 1
...
CREATE OR REPLACE VIEW `informea_country_reports_title` AS
  SELECT
    ...
    'en'              AS 'language',
    b.title
  FROM informea_country_reports a INNER JOIN `prod_ascobans`.node b ON a.id = b.uuid;
...
CREATE OR REPLACE VIEW informea_documents AS
  SELECT
    ...
    node.nid
  FROM `prod_ascobans`.node node
    LEFT JOIN `prod_ascobans`.field_data_field_publication_published_date pdate ON node.nid = pdate.entity_id
    LEFT JOIN `prod_ascobans`.field_data_field_publication_image img ON node.nid = img.entity_id
    LEFT JOIN `prod_ascobans`.file_managed thumbnails ON field_publication_image_fid = thumbnails.fid
    LEFT JOIN `prod_ascobans`.field_data_field_country country ON country.entity_id = node.nid
    LEFT JOIN `prod_ascobans`.field_data_field_country_iso3 ciso ON country.field_country_target_id = ciso.entity_id
  WHERE
...
CREATE OR REPLACE VIEW informea_documents_types AS
  ...
  SELECT
    ...
    tb.name `value`
  FROM informea_documents a
    INNER JOIN `prod_ascobans`.field_data_field_publication_type b ON a.nid = b.entity_id
    INNER JOIN `prod_ascobans`.taxonomy_term_data tb ON b.field_publication_type_tid = tb.tid
...
CREATE OR REPLACE VIEW informea_documents_authors AS
  SELECT
    ...
    tb.name
  FROM informea_documents a
    INNER JOIN `prod_ascobans`.field_data_field_publication_author b ON a.nid = b.entity_id
    INNER JOIN `prod_ascobans`.taxonomy_term_data tb ON tb.tid = b.field_publication_author_tid;
...
CREATE OR REPLACE VIEW informea_documents_keywords AS
  ...
  FROM informea_documents a
    ...
    INNER JOIN `prod_ascobans`.taxonomy_term_data td ON itags.field_related_informea_terms_target_id = td.tid;
...
CREATE OR REPLACE VIEW informea_documents_title AS
  ...
  FROM informea_documents a
    INNER JOIN `prod_ascobans`.field_data_title_field b ON a.nid = b.entity_id
...
CREATE OR REPLACE VIEW informea_documents_files AS
  SELECT
    ...
    files.filename
  FROM informea_documents a
    INNER JOIN `prod_ascobans`.field_data_field_publication_attachment f ON a.nid = f.entity_id
    INNER JOIN `prod_ascobans`.file_managed files ON f.field_publication_attachment_fid = files.fid
  GROUP BY files.fid;
...
CREATE OR REPLACE VIEW `informea_documents_references` AS
  SELECT
    ...
    FROM
      ...
      JOIN `prod_ascobans`.field_data_field_publication_meeting b ON a.nid = b.entity_id
      JOIN `prod_ascobans`.node bn ON (b.field_publication_meeting_target_id = bn.nid AND bn.type = 'meeting')
    GROUP BY bn.nid
  UNION
    SELECT
    ...
    FROM
      informea_documents a
      JOIN `prod_ascobans`.field_data_field_publication_plans b ON a.nid = b.entity_id
      JOIN `prod_ascobans`.node bn ON (b.field_publication_plans_target_id = bn.nid and (bn.type = 'document'))
    GROUP BY bn.nid
  UNION
    SELECT
    ...
    FROM
      informea_documents a
      JOIN `prod_ascobans`.field_data_field_publication_nat_report b ON a.nid = b.entity_id
      JOIN `prod_ascobans`.node bn ON (b.field_publication_nat_report_target_id = bn.nid AND bn.type = 'document')
    GROUP BY bn.nid;
...
CREATE OR REPLACE VIEW `informea_contacts` AS
  ...
  FROM `prod_ascobans`.odata_focal_point a;
