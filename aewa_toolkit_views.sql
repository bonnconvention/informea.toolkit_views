...
CREATE OR REPLACE DEFINER=`edw_www`@`localhost` SQL SECURITY DEFINER VIEW `informea_meetings` AS
    ...
    FROM
        ...
        INNER JOIN `edw_aewa_drupal`.field_data_field_country i ON a.nid = i.entity_id
...
CREATE OR REPLACE DEFINER=`edw_www`@`localhost` SQL SECURITY DEFINER VIEW `informea_meetings_title` AS
    SELECT
        ...
        'en' AS 'language',
        a.title
    FROM `edw_aewa_drupal`.node a WHERE a.`type` = 'meeting';
...
CREATE OR REPLACE DEFINER=`edw_www`@`localhost` SQL SECURITY DEFINER VIEW `informea_decisions` AS
    ...
    FROM `edw_aewa_drupal`.node a
        ...
        INNER JOIN `edw_aewa_drupal`.informea_decisions_cop_documents g ON g.id_document = a.nid
    WHERE
        a.`type`='document'
...
CREATE OR REPLACE DEFINER=`edw_www`@`localhost` SQL SECURITY DEFINER VIEW `informea_country_reports` AS
    ...
    FROM `edw_aewa_drupal`.node a
        ...
        INNER JOIN `edw_aewa_drupal`.node e1 ON e.field_instrument_target_id = e1.nid
        INNER JOIN `edw_aewa_drupal`.field_data_field_document_publish_date f ON f.entity_id = a.nid
        INNER JOIN `edw_aewa_drupal`.field_data_field_country g ON g.entity_id = a.nid
